...
export const style = (values: Values | null): cytoscape.Stylesheet[] => {
  ...
  if (values === null) {
    ...
    const rawValues = Object.entries(values).map((value) => value[1]['value']['time']);
    const sequentialScheme = d3
      .scaleSequential(d3.interpolateReds)
      .domain([Math.min(...rawValues), Math.max(...rawValues)]);
    colors = Object.entries(values).map((value, index) => {
      return {
        selector: `node[id="${value[0]}"]`,
        style: {'background-color': sequentialScheme(rawValues[index])},
      };
    });
  }
  return [
