...
import {createScanner, SyntaxKind, ScanError, getLocation} from 'jsonc-parser';
import {
  TextDocument,
  FoldingRangeKind,
  FoldingRange,
...
export function getFoldingRanges(document: TextDocument, context?: FoldingRangesContext): FoldingRange[] {
  ...
  const original_text: string = document.getText();
  ...
  while (token !== SyntaxKind.EOF) {
    switch (token) {
      ...
      case SyntaxKind.OpenBracketToken: {
        ...
        const offset = scanner.getPosition();
        const loc = getLocation(original_text, offset - 1);
      ...
      case SyntaxKind.CloseBracketToken: {
        ...
        if (stack.length > 0 && stack[stack.length - 1].kind === kind) {
          ...
          // same line object that belongs to an array
          if (range && line == range.startLine && range.kind === 'object' && stack[stack.length - 1].kind === 'array') {
            range.kind = 'object';
            addRange(range);
          }
